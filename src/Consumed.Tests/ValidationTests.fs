...
let ``Validating consume command without description returns argument empty``() = 
    validate ( Consume { Category = "book"; Description = ""; Url = "http://gameofthrones.com" } )
...
let ``Validating consume command with category that does not exist returns out of range``() = 
    let actual = validate ( Consume { Category = "i_do_not_exist"; Description = "Game of Thrones"; Url = "http://gameofthrones.com" } )
    match actual with
...
let ``Validating consume command without url returns argument empty``() = 
    validate ( Consume { Category = "book"; Description = "Game of Thrones"; Url = "" } )
    |> assertReturnsEmptyArgument "url"
    
[<Test>]
let ``Validating consume command with url in wrong structure returns argument structure``() = 
    let actual = validate ( Consume { Category = "book"; Description = "Game of Thrones"; Url = "gameofthrones.com" } )
    match actual with
